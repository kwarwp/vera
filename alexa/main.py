# vera.alexa.main.py
from _spy.vitollino.main import Cena,STYLE,Codigo, INVENTARIO, Texto, Elemento
...
class Coisa:
    def __init__(self):
        pass
    def click(self, ev):
        Texto(self.parque, self.tit).vai()
        score(casa=[ev.x, ev.y], carta=self.tit, move="CLICK", ponto="OK", valor="COISA", _level=1)
...
class Crianca:
    def __init__(self, parque, tit="crianca", x=0, y=210, gosta=""):
        self.parque, self.tit = parque, tit
        dragger = {ti: lambda e,t,i=ti in gosta: parque.gostou(e, t, tit, ok=i) for ti in ALL}
        crianca = Elemento(CRIANCA, tit=tit, x=x-10, y=y-10, w=90, h=140, style={"opacity": 0.3}, drop=dragger, vai=self.click)
        crianca.entra(parque)
        
    def click(self, ev):
        Texto(self.parque, f"Eu {self.tit}").vai()
